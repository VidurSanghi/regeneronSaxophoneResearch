...
# Simple code with Droupout Regularization and Seeding for stability, to perform feature set analysis, and neural network model training ...
from google.colab import drive
import os
import numpy as np
import librosa
...
# Initiatilze random seed for more consistent outputs
seed_value = 42
seed(seed_value)
...
# Build the neural network model with added dropout layers
# Note: This code adds dropout layers with a dropout rate of 30% after the first and second hidden layers.
# Adjust the dropout rate as necessary (between 20% to 50%) to improve the model's generalization while
# minimizing overfitting.

model = Sequential([
    Dense(64, activation='relu', input_shape=(combined_features.shape[1],)),
    #Dropout(0.25),  # Set dropout to 25%
    Dense(32, activation='relu'),
    #Dropout(0.2),  # Set dropout to 20%
    Dense(1, activation='sigmoid')
])
...
# Perform a thresholding to map the continuous predictions to discrete labels
threshold = 0.5    # may vary threshold
predicted_labels = (test_labels > threshold).astype(int)
